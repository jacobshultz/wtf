...
from wtf_data import Settings, get_usr_prompt, get_sys_prompt

available_tools = {'web_search': web_search, 'web_fetch': web_fetch}

class OllamaClient:
    @staticmethod
    def query(cmd: str, errCode: str, settings: Settings) -> str | None:
        s_prompt = get_sys_prompt()
        u_prompt = get_usr_prompt(cmd, errCode)
        ...
        if msg is not None: 
            return msg.splitlines()[-1] if not settings.Debug else msg
        else: 
            return